export function icomesh (order: number = 4) : Mesh {
  ...
  const numDuplicates = order === 0 ? 3 : 2 ** order * 3 + 9

  const vertices:Float32Array = new Float32Array((numVertices + numDuplicates) * 3)
  vertices.set(Float32Array.of(
    -1, f, 0, 1, f, 0, -1, -f, 0, 1, -f, 0,
    0, -1, f, 0, 1, f, 0, -1, -f, 0, 1, -f,
    f, 0, -1, f, 0, 1, -f, 0, -1, -f, 0, 1
  ))
  let triangles = (order > 5 ? Uint32Array : Uint16Array).of(
  ...
  function addMidPoint (a, b) {
    ...
    const i = midCache ? midCache.get(key) : undefined
    if (i !== undefined) {
